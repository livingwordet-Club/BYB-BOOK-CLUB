...
const authenticateToken = (req: any, res: any, next: any) => {
  ...
  jwt.verify(token, JWT_SECRET, (err: any, decoded: any) => {
    ...
    if (!userExists) return res.status(401).json({ error: "User no longer exists." });
    req.user = decoded;
    next();
  });
};
...
app.put("/api/user/profile", authenticateToken, (req: any, res) => {
  ...
  db.prepare("UPDATE users SET name = ?, email = ?, bio = ?, profile_verse = ?, profile_pic = ? WHERE id = ?").run(name, email, bio, profile_verse, profile_pic, req.user.id);
...
app.get("/api/dashboard", authenticateToken, (req: any, res) => {
  ...
  const currentRead = db.prepare(`SELECT b.* FROM books b JOIN user_reads ur ON b.id = ur.book_id WHERE ur.user_id = ? ORDER BY ur.last_read_at DESC LIMIT 1`).get(req.user.id);
...
app.post("/api/books", authenticateToken, (req: any, res) => {
    ...
    db.prepare("INSERT INTO books (title, author, cover_url, content, is_trending, is_new) VALUES (?, ?, ?, ?, ?, ?)").run(title, author, cover_url, content, is_trending ? 1 : 0, is_new ? 1 : 0);
    res.json({ success: true });
});

app.get("/api/messages", authenticateToken, (req: any, res) => {
    const messages = db.prepare(`SELECT m.*, u.username as sender_name FROM messages m JOIN users u ON m.sender_id = u.id WHERE receiver_id = ? OR sender_id = ? ORDER BY created_at ASC`).all(req.user.id, req.user.id);
    res.json(messages);
});

app.post("/api/messages", authenticateToken, (req: any, res) => {
    const { receiver_id, content } = req.body;
    db.prepare("INSERT INTO messages (sender_id, receiver_id, content) VALUES (?, ?, ?)").run(req.user.id, receiver_id, content);
    res.json({ success: true });
});
...
app.post("/api/activity", authenticateToken, (req: any, res) => {
    ...
    db.prepare("INSERT INTO user_activity (user_id, type, content, metadata) VALUES (?, ?, ?, ?)").run(req.user.id, type, content, JSON.stringify(metadata));
    res.json({ success: true });
});

// --- ADMIN & AUDIOBOOK ROUTES ---
app.get("/api/admin/stats", authenticateToken, (req: any, res) => {
    if (!req.user.isAdmin) return res.sendStatus(403);
    const userCount = db.prepare("SELECT COUNT(*) as count FROM users").get();
    const bookCount = db.prepare("SELECT COUNT(*) as count FROM books").get();
    const recentActivity = db.prepare(`SELECT ua.*, u.username FROM user_activity ua JOIN users u ON ua.user_id = u.id ORDER BY created_at DESC LIMIT 20`).all();
...
app.delete("/api/user/delete", authenticateToken, (req: any, res) => {
  ...
  try { deleteTransaction(); res.json({ success: true }); } 
  catch (error) { res.status(500).json({ error: "Failed to delete account" }); }
});

// --- SERVER START & PRODUCTION STATIC SERVING ---
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    // FIX: Serving from __dirname because server.js is inside dist
